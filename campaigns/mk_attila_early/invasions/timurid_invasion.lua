...
------------------------------------------------------------------------------------------------------------------------
--
-- 	MEDIEVAL KINGDOMS 1212 - TIMURID INVASION
-- 	By: DETrooper
...
TIMURID_NUM_CAVALRY_ARMIES = 8;
TIMURID_NUM_SIEGE_ARMIES = 4;
TIMURID_ADD_ARMIES_PER_DIFFICULTY_LEVEL = 2;
TIMURID_FREE_UPKEEP_TIME = 25;
...
function FactionTurnStart_Timurid_Preservation(context)
	...
	elseif turn_number == TIMURID_INVASION_TURN and TIMURID_INVASION_STARTED == false then
		...
		for i = 1, TIMURID_NUM_CAVALRY_ARMIES - (difficulty * TIMURID_ADD_ARMIES_PER_DIFFICULTY_LEVEL) do
			SpawnTimuridArmyInZone(TIMURIDS_KEY, Invasion_Build_Unit_List(JOCHI_KEY, "cavalry"), region, spawn_zone);
		end

		for i = 1, TIMURID_NUM_SIEGE_ARMIES - (difficulty * TIMURID_ADD_ARMIES_PER_DIFFICULTY_LEVEL) do
			SpawnTimuridArmyInZone(TIMURIDS_KEY, Invasion_Build_Unit_List(JOCHI_KEY, "siege"), region, spawn_zone);
		end
...
function TimuridArmyChecks(faction_name)
	...
	if faction_name == TIMURIDS_KEY then
		...
		if forces:num_items() < MIN_ARMY_STRENGTH_FORCES then
			for i = 1, 2 do
				SpawnArmyInZone(faction_name, army, region, spawn_zone, {key = "mk_bundle_army_timurid_invasion", turns = 25});
			end
