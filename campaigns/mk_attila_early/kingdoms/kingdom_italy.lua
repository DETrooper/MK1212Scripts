...
function Add_Kingdom_Italy_Listeners()
	...
	if cm:is_multiplayer() == false then
		...
		if cm:is_new_game() then
			...
			if table.HasValue(ITALIAN_FACTIONS, faction_name) then
				mkDecisions:Add_Decision("form_kingdom_italy", faction_name, false, true);
			end
		end
	end
end
...
function Italian_Regions_Check(context)
	...
	if has_regions == true then
		...
		elseif not mkHRE or faction_name ~= mkHRE.emperor_key then
			mkDecisions:Enable_Decision("form_kingdom_italy");
		end
	end
end
...
function Italian_Kingdom_Formed(faction_name)
	...
	if cm:is_multiplayer() == false then
		mkDecisions:Remove_Decision("form_kingdom_italy");
