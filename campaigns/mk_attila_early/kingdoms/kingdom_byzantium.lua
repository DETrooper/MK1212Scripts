	...
	if cm:is_multiplayer() == false then
		mkDecisions:Register_Decision(
			...
			function() 	
				...
				if cm:model():world():region_manager():region_by_key("mk_reg_constantinopolis"):owning_faction():name() == cm:get_local_faction() then
		...
		if cm:is_new_game() then
			...
			if faction_name == EPIRUS_KEY or faction_name == NICAEA_KEY or faction_name == TREBIZOND_KEY then
				mkDecisions:Add_Decision("restore_byzantine_empire", faction_name, false, true);
			end
		end
	end
end
...
function Constantinople_Check(faction_name)
	local region = cm:model():world():region_manager():region_by_key("mk_reg_constantinopolis");
	...
	if region:owning_faction():name() == faction_name then
		...
		if cm:is_multiplayer() == false and cm:model():world():faction_by_key(faction_name):is_human() == true then
			mkDecisions:Disable_Decision("restore_byzantine_empire");
		end
	end
end
...
function Constantinople_Check_Occupied(context)
	...
	if faction_name == EPIRUS_KEY or faction_name == NICAEA_KEY or faction_name == TREBIZOND_KEY then
		if context:character():region():name() == "mk_reg_constantinopolis" then
			Constantinople_Taken(faction_name);
		end
	else
		if context:character():region():name() == "mk_reg_constantinopolis" and cm:is_multiplayer() == false and context:character():faction():is_human() == true then
			mkDecisions:Disable_Decision("restore_byzantine_empire");
		end
	end
end
...
function Constantinople_Taken(faction_name)
	...
	elseif (not mkHRE or (mkHRE and table.HasValue(mkHRE.factions, faction_name) ~= true)) then
		mkDecisions:Enable_Decision("restore_byzantine_empire");
	end
end
...
function Byzantine_Empire_Restored(faction_name)
	...
	if cm:is_multiplayer() == false then
		mkDecisions:Remove_Decision("restore_byzantine_empire");

		if cm:model():world():faction_by_key(faction_name):is_human() then
			mkDecisions:Add_Decision("restore_roman_empire", faction_name, false, true);
		end
	end
