	...
	if cm:is_multiplayer() == false then
		mkDecisions:Register_Decision(
			...
			function() 
				...
				if cm:model():world():region_manager():region_by_key("mk_reg_constantinopolis"):owning_faction():name() == BYZANTINE_EMPIRE_FACTION then
					conditionstring = conditionstring.."([[rgba:8:201:27:150]]Y[[/rgba]]) - Own the region of "..REGIONS_NAMES_LOCALISATION["mk_reg_constantinopolis"]..".\n";
				else
					conditionstring = conditionstring.."([[rgba:255:0:0:150]]X[[/rgba]]) - Own the region of "..REGIONS_NAMES_LOCALISATION["mk_reg_constantinopolis"]..".\n";
				end
...
function Byzantium_Regions_Check(faction_name)
	...
	if cm:model():world():region_manager():region_by_key("mk_reg_constantinopolis"):owning_faction():name() == faction_name then
	...
	if ROMAN_EMPIRE_RESTORED == false and LATIN_EMPIRE_DEAD == true and has_constantinople == true and has_regions_africa == true and has_regions_anatolia == true and has_regions_dalmatia == true and has_regions_egypt == true and has_regions_italy == true and has_regions_greece == true and has_regions_oriens == true then
		...
		elseif (not mkHRE or (mkHRE and table.HasValue(mkHRE.factions, faction_name) ~= true)) then
			mkDecisions:Enable_Decision("restore_roman_empire");
		end
	end
end
...
function Roman_Empire_Restored(faction_name)
	...
	if cm:is_multiplayer() == false then
		mkDecisions:Remove_Decision("restore_roman_empire");
