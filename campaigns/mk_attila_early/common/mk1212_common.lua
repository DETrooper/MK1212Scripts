...
function CharacterPerformsOccupationDecisionLootSack_Global(context)
	...
	if region then
		if not table.HasValue(SACKED_SETTLEMENTS_TOTAL[faction_name], region:name()) then
...
function Check_Razed_Regions()
	...
	for i = 0, region_list:num_items() - 1 do
		...
		if region:owning_faction():name() == "rebels" then
			if not table.HasValue(REGIONS_RAZED, region_name) then
				table.insert(REGIONS_RAZED, region_name);
			end
		elseif table.HasValue(REGIONS_RAZED, region_name) then
...
function SpawnArmyInZone(faction_name, unit_list, region, zone, effect_bundle, callback)
	...
	cm:create_force(
		...
		function(cqi)
			if effect_bundle then
				if istable(effect_bundle) then
					cm:apply_effect_bundle_to_characters_force(effect_bundle.key, cqi, effect_bundle.turns, true);
				else
					cm:apply_effect_bundle_to_characters_force(effect_bundle, cqi, 10, true);
				end
			end
...
function GetTurnFromYear(year)
	local turn_number = ((year - 1212) * TURNS_PER_YEAR) + 1;

	return turn_number;
end

function GetYearFromTurn(turn, bRound)
	local year_number = 1212 + ((turn - 1) / TURNS_PER_YEAR);

	if bRound then
		year_number = math.floor(year_number);
	end
...
function LoadKeyPairTables(context, savename)
	...
	if savestring ~= "" then
		...
		for i = 1, #first_split do
			...
			if #tab2 > 0 then
				tab[second_split[1]] = table.DeepCopy(tab2);
			end
		end
	end
...
function table.Random(tab)
	local randomKey = math.random(1, table.Count(tab));
	local i = 1;
