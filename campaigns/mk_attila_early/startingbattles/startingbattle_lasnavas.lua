...
function Add_Las_Navas_Listeners()
	...
	if almohads:is_human() == true or aragon:is_human() == true or castile:is_human() == true or navarre:is_human() == true then
		if BATTLE_LAS_NAVAS_OCCURED == false then
			...
			cm:add_listener(
				"PanelClosedCampaign_Las_Navas",
				"PanelClosedCampaign",
				true,
				function(context) PanelClosedCampaign_Las_Navas(context) end,
				true
			);
		end

		-- Make sure it happens after any cutscene.
		cm:register_ui_created_callback(function()
			if cm:is_new_game() and not intro_cinematic_str then
...
function PanelClosedCampaign_Las_Navas(context)
