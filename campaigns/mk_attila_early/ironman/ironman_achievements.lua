			...
			if achievement then
				...
				if achievement.unlocked == false and achievement.manual ~= true then
						...
						if achievement.requirednumbuildings then
							...
							for i = 0, faction_region_list:num_items() - 1 do
								...
								for j = 0, buildings_list:num_items() - 1 do
									if table.HasValue(achievement.requiredbuildings, buildings_list:item_at(j):name()) then
					...
					if achievement.requiredfactions then
						if not table.HasValue(achievement.requiredfactions, faction_name) then
					...
					if achievement.sacksettlements then
						for i = 1, #achievement.sacksettlements do
							if not table.HasValue(SACKED_SETTLEMENTS_TOTAL[faction_name], achievement.sacksettlements[i]) then
					...
					if achievement.requiredvassals then
						if FACTIONS_TO_FACTIONS_VASSALIZED[faction_name] then
							for i = 1, #achievement.requiredvassals do
								if not table.HasValue(FACTIONS_TO_FACTIONS_VASSALIZED[faction_name], achievement.requiredvassals[i]) then
