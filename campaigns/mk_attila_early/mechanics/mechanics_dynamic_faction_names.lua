	...
	if cm:is_multiplayer() == false then
		...
		mkDecisions:Register_Decision(
			...
			function() 	
				...
				if mkHRE and table.HasValue(mkHRE.factions, faction_name) then
		...
		for i = 1, #DYNAMIC_FACTION_NAMES_FACTIONS do
			...
			if faction:is_null_interface() == false then
				...
				if cm:is_multiplayer() == false and faction:is_human() then
					if not table.HasValue(FACTIONS_DFN_KINGDOMS_EVENTS, faction_name) and FACTIONS_DFN_LEVEL[faction_name] < 2 then
						mkDecisions:Add_Decision("found_a_kingdom", faction_name, false, false);
					end

					if not table.HasValue(FACTIONS_DFN_EMPIRES_EVENTS, faction_name) and FACTIONS_DFN_LEVEL[faction_name] < 3 then
						mkDecisions:Add_Decision("found_an_empire", faction_name, false, false);
					end
				end
			end
		end
		...
		if not faction:is_null_interface() then
			...
			if FactionIsAlive(faction_name) then
				if FACTIONS_DFN_LEVEL[faction_name] == 1 then
					if not table.HasValue(FACTIONS_DFN_KINGDOMS_EVENTS, faction_name) then
						if faction:region_list():num_items() >= NUM_REQUIRED_REGIONS_LVL2 and (not mkHRE or faction_name ~= mkHRE.emperor_key) then
							if faction:is_human() == false or cm:is_multiplayer() == true then
								...
								mkDecisions:Enable_Decision("found_a_kingdom");
							end
						else
							if faction:is_human() == true and cm:is_multiplayer() == false then
								mkDecisions:Disable_Decision("found_a_kingdom");
							end
						end
					end
				elseif FACTIONS_DFN_LEVEL[faction_name] == 2 then
					if not table.HasValue(FACTIONS_DFN_EMPIRES_EVENTS, faction_name) then
						if faction:region_list():num_items() >= NUM_REQUIRED_REGIONS_LVL3 and (not mkHRE or (mkHRE and not table.HasValue(mkHRE.factions, faction_name))) then
							if faction:is_human() == false or cm:is_multiplayer() == true then
								DFN_Set_Faction_Rank(faction_name, 3);
							else
								mkDecisions:Enable_Decision("found_an_empire");
							end
						else
							if faction:is_human() == true and cm:is_multiplayer() == false then
								mkDecisions:Disable_Decision("found_an_empire");
							end
						end
					end
...
function DFN_Set_Faction_Rank(faction_name, set_rank)
	...
	if cm:is_multiplayer() == false then
		if rank == 2 or rank == 4 then
			mkDecisions:Remove_Decision("found_a_kingdom");
		elseif rank == 3 or rank == 5 then
			mkDecisions:Remove_Decision("found_an_empire");
		end
	end
