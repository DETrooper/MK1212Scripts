...
function mkHRE:Add_Region_Listeners()
	...
	if cm:is_new_game() then
		for i = 1, #self.regions do
			...
			if table.HasValue(self.factions, region_owning_faction_name) then
	...
	if not self.destroyed then
		...
		for i = 1, #self.regions do
			...
			if table.HasValue(self.factions, region_owning_faction_name) then
				...
				if not table.HasValue(self.regions_in_empire, region_name) then
		...
		for i = 1, #self.regions_in_empire do
			...
			if not table.HasValue(regions_in_empire, region_name) then
...
function mkHRE:Region_Reconquered(region_name)
	...
	elseif table.HasValue(self.factions, faction_name) then
		cm:show_message_event(
			faction_name,
			"message_event_text_text_mk_event_hre_region_reconquered_title",
			"regions_onscreen_"..region_name,
