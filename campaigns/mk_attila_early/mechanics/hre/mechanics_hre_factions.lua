...
mkHRE.frankfurt_region_key = "mk_reg_frankfurt";
...
function mkHRE:Add_Faction_Listeners()
	...
	if cm:is_new_game() then
		self.factions = table.DeepCopy(self.factions_start);
		self.factions_to_states = table.DeepCopy(self.factions_to_states_start);
...
function FactionTurnStart_HRE_Factions(context)
	...
	if faction_name == mkHRE.emperor_key and mkHRE.current_reform < 9 then
		...
		for i = 0, faction_list:num_items() - 1 do
			...
			if table.HasValue(mkHRE.factions, current_faction_name) and current_faction_name ~= mkHRE.emperor_key then
...
function CharacterBecomesFactionLeader_HRE_Factions(context)
	...
	if faction_name ~= mkHRE.emperor_key and table.HasValue(mkHRE.factions, faction_name) and not context:character():faction():is_human() then
...
function FactionBecomesLiberationVassal_HRE_Factions(context)
	if context:liberating_character():faction():name() == mkHRE.emperor_key then
		if table.HasValue(mkHRE.factions_start, context:faction():name()) then
...
function GarrisonAttackedEvent_HRE_Factions(context)
	if table.HasValue(mkHRE.regions, context:garrison_residence():region():name()) and context:character():faction():name() == mkHRE.emperor_key then
		if mkHRE.liberation_disabled == true then
			mkHRE.liberation_disabled = false;
		end
...
function GarrisonOccupiedEvent_HRE_Frankfurt(context)
	...
	if frankfurt_owner_name  then
		if table.HasValue(mkHRE.factions, frankfurt_owner_name) then
	...
	if mkHRE.regions_owners[region_name] ~= region_owning_faction_name then
		if table.HasValue(mkHRE.factions, mkHRE.regions_owners[region_name]) == true and region_owning_faction_name ~= mkHRE.emperor_key then
...
function TimeTrigger_HRE_Factions(context)
	if context.string == "hre_liberation_check" then
		...
		if not table.HasValue(mkHRE.factions, mkHRE.liberated_faction) and table.HasValue(mkHRE.factions_start, mkHRE.liberated_faction) then
	...
	elseif context.string == "hre_frankfurt_transfer_delay" then
		...
		if owning_faction_name ~= mkHRE.emperor_key and table.HasValue(mkHRE.factions, owning_faction_name) then
	...
	if faction_state ~= "puppet" then
		if emperor_faction:is_human() then
			if FACTIONS_TO_FACTIONS_VASSALIZED[self.emperor_key] then
				if table.HasValue(FACTIONS_TO_FACTIONS_VASSALIZED[self.emperor_key], faction_name) then
		...
		for i = 0, faction_list:num_items() - 1 do
			...
			if faction:allied_with(possible_ally) == true and table.HasValue(self.factions, possible_ally:name()) then
...
function mkHRE:Check_Factions_In_Empire()
	if not self.destroyed then
		local hre_factions_copy = table.DeepCopy(self.factions);
		...
		for i = 1, #self.factions_start do
			...
			if FactionIsAlive(faction_name) and table.HasValue(self.factions, faction_name) == false then
...
function mkHRE:Replace_Emperor(faction_name)
	...
	for i = 1, #self.factions do
		if old_emperor_faction then
			if self.factions[i] == old_emperor then
				if not table.HasValue(self.factions_start, old_emperor) then
	...
	if not table.HasValue(self.factions, faction_name) then
...
function mkHRE:Assign_New_Pretender(exclude_player)
	...
	for i = 0, faction_list:num_items() - 1 do
		...
		if table.HasValue(self.factions, current_faction_name) ~= true and current_faction:state_religion() == "att_rel_chr_catholic" and current_faction:is_horde() == false then
...
function mkHRE:HRE_Vanquish_Pretender()
	if self.emperor_pretender_key ~= "nil" and self.emperor_pretender_key ~= self.emperor_key then
		...
		if not table.HasValue(self.factions_start, self.emperor_pretender_key) then
...
function mkHRE:Get_Faction_State(faction_name)
	if table.HasValue(self.factions, faction_name) then
