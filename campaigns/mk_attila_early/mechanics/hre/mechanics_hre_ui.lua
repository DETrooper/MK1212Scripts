	...
	if hre_panel_open == true then
		...
		elseif string.find(context.string, "_Reform_Button") then
			...
			if cm:get_local_faction() == mkHRE.emperor_key then
				...
				if table.HasValue(mkHRE.reforms_votes, cm:get_local_faction()) then
		...
		elseif string.find(context.string, "_logo") then
			...
			if string.find(context.string, "mk_fact_") then
				...
				if table.HasValue(mkHRE.factions, faction_name) and faction_name ~= mkHRE.emperor_key then
			...
			elseif string.find(context.string, "mk_reg_") then
				...
				if table.HasValue(mkHRE.factions, owning_faction_name) and owning_faction_name ~= mkHRE.emperor_key then
...
function mkHRE:OpenHREPanel()
	...
	if self.emperor_pretender_key ~= "nil" then
		...
		if table.HasValue(FACTIONS_WITH_IMAGES, self.emperor_pretender_key) then
	...
	if table.HasValue(FACTIONS_WITH_IMAGES, self.emperor_key) then
		...
		if self.current_reform >= i then
			...
			if self.current_reform == i - 1 then
				if cm:get_local_faction() == self.emperor_key then
					...
					if table.HasValue(self.factions, cm:get_local_faction()) then
...
function mkHRE:Setup_Faction_Info_UI(root, faction_name)
	...
	elseif table.HasValue(self.elector_factions, faction_name) then
		in_hre = UI_LOCALISATION["hre_in_hre_elector"];
	elseif not table.HasValue(self.factions, faction_name) then
	...
	if table.HasValue(FACTIONS_WITH_IMAGES, faction_name) then
	...
	if table.HasValue(self.factions, faction_name) or self.emperor_pretender_key == faction_name then
		...
		if faction_name == self.emperor_pretender_key or (self.current_reform > 0 and table.HasValue(self.elector_factions, faction_name) == false) then
			child_2:SetVisible(false);
		else
			child_2:SetVisible(true);
		end

		if table.HasValue(FACTIONS_WITH_LEADER_IMAGES, faction_name) then
...
function mkHRE:Update_State_UI(faction_name)
	...
	if table.HasValue(self.factions, faction_name) and faction_name ~= self.emperor_key and cm:model():world():faction_by_key(faction_name):is_human() == false then
...
function mkHRE:Setup_Elector_Faction_Info_UI(root, info_faction_name)
	...
	if faction_name and FactionIsAlive(faction_name) then
		...
		if info_faction_name ~= self.emperor_pretender_key then
			if (self.current_reform == 0 or (self.current_reform > 0 and table.HasValue(self.elector_factions, info_faction_name))) and self.current_reform < 8 then
				if table.HasValue(FACTIONS_WITH_LEADER_IMAGES, faction_name) then
		...
		if self.current_reform < 8 then
			if info_faction_name == self.emperor_pretender_key then
				...
				if not table.HasValue(self.factions, cm:get_local_faction()) then
					btnVote:SetVisible(false);
					return;
				end

				if self.current_reform == 0 or (self.current_reform > 0 and table.HasValue(self.elector_factions, human_faction_name)) then
		...
		if cm:get_local_faction() ~= self.emperor_pretender_key and (self.current_reform == 0 or (self.current_reform > 0 and table.HasValue(self.elector_factions, human_faction_name))) then
			btnVote:SetVisible(true);
			btnVote:SetMoveable(true);
			btnVote:MoveTo(parchment_uicX + 193, parchment_uicY + 644);
			btnVote:SetMoveable(false);
