...
function Add_Population_Listeners()
	...
	if cm:is_new_game() then
		POPULATION_REGIONS_POPULATIONS = table.DeepCopy(POPULATION_REGIONS_STARTING_POPULATIONS);
		POPULATION_REGIONS_MANPOWER = table.DeepCopy(POPULATION_REGIONS_STARTING_POPULATIONS);
		...
		for i = 0, region_list:num_items() - 1 do
			...
			if not string.find(region_name, "terra_incognita") then
				for j = 1, 5 do
...
function Compute_Region_Growth(region)
	...
	for i = 1, 5 do
		...
		if hre_decree then
			if table.HasValue(mkHRE.factions, region_owning_faction_name) then
				if i == 2 or i == 3 then
...
function Check_Technologies_Population(faction_name)
	...
	for k, v in pairs(POPULATION_TECHNOLOGIES_TO_GROWTH) do
		if not table.HasValue(POPULATION_FACTION_TECH_RESEARCHED[faction_name], k) then
