...
function Add_Crusade_Event_Listeners()
	...
	cm:add_listener(
		"FactionReligionConverted_Crusades",
		"FactionReligionConverted",
		true,
		function(context) FactionReligionConverted_Crusades(context) end,
		true
	);
	cm:add_listener(
	...
	elseif MISSION_TAKE_JERUSALEM_ACTIVE == true then
		...
		cm:add_listener(
			"MissionFailed_Jerusalem",
			"MissionFailed",
			true,
			function(context) MissionFailed_Jerusalem(context) end,
			true
		);
	end
end
...
function FactionTurnStart_Pope_Crusades(context)
	if PAPAL_FAVOUR_SYSTEM_ACTIVE == true then
		...
		if (CURRENT_CRUSADE < MAX_NUM_OF_CRUSADES or CRUSADE_ACTIVE == true) then
			if context:faction():is_human() and cm:model():turn_number() >= NEXT_CRUSADE_MESSAGE_TURN and NEXT_CRUSADE_MESSAGE_TURN ~= -1 then
				...
				if table.HasValue(CRUSADE_TARGET_RELIGIONS, owner_religion) then
			...
			if CRUSADE_ACTIVE == true then
				...
				if context:faction():is_human() then
					if true_owner:state_religion() == "att_rel_chr_catholic" then
						cm:override_mission_succeeded_status(faction_name, CURRENT_CRUSADE_MISSION_KEY, true);
						End_Crusade("won");
					...
					if cm:model():turn_number() == NEXT_CRUSADE_START_TURN + CRUSADE_DURATION then
						...
						if target_faction:is_human() == false then
							cm:override_mission_succeeded_status(faction_name, CURRENT_CRUSADE_MISSION_KEY, false);
						end
					end
				else
					if table.HasValue(CURRENT_CRUSADE_FACTIONS_JOINED, faction_name) and context:faction():state_religion() ~= "att_rel_chr_catholic" then
						Remove_Faction_From_Crusade(faction_name);
					elseif faction_name == CURRENT_CRUSADE_TARGET and context:faction():state_religion() == "att_rel_chr_catholic" then
						End_Crusade("aborted");
					end

					if SCRIPTED_CRUSADES_LIST[tostring(CURRENT_CRUSADE)] then
						if table.HasValue(SCRIPTED_CRUSADES_LIST[tostring(CURRENT_CRUSADE)][6], faction_name) then
							if context:faction():state_religion() == "att_rel_chr_catholic" and context:faction():at_war_with(owner) == false and table.HasValue(FACTION_EXCOMMUNICATED, faction_name) ~= true then
								cm:force_declare_war(faction_name, owner:name());
							end
						...
						if context:faction():state_religion() == "att_rel_chr_catholic" and table.HasValue(FACTION_EXCOMMUNICATED, faction_name) ~= true then
							if faction_name == JERUSALEM_KEY or context:faction():region_list():num_items() >= CRUSADE_CANDIDATE_MIN_REGIONS then
		...
		if MISSION_TAKE_JERUSALEM_ACTIVE then
			if context:faction():is_human() then
				local region = cm:model():world():region_manager():region_by_key(JERUSALEM_REGION_KEY);
				...
				if region:owning_faction():name() == faction_name then
					if context:faction():state_religion() == "att_rel_chr_catholic" then
						...
						cm:override_mission_succeeded_status(faction_name, "mk_mission_crusades_take_jerusalem_dilemma", true);
						cm:add_time_trigger("Transfer_Jerusalem_Crusades", 0.5);
						cm:remove_listener("CharacterEntersGarrison_Jerusalem");
						cm:remove_listener("MissionFailed_Jerusalem");
					end
				end
			end
		end
...
function CharacterCompletedBattle_Crusades(context)
	...
	if table.HasValue(CURRENT_CRUSADE_FACTIONS_JOINED, character_faction_name) then
	...
	elseif character_faction_name == CURRENT_CRUSADE_TARGET_OWNER or character:allied_with(cm:model():world():faction_by_key(CURRENT_CRUSADE_TARGET_OWNER)) then
		if pending_battle:attacker():faction():name() == character_faction_name then
			if not table.HasValue(CURRENT_CRUSADE_FACTIONS_JOINED, pending_battle:defender():faction():name()) then
				return;
			end
		elseif pending_battle:defender():faction():name() == character_faction_name then
			if not table.HasValue(CURRENT_CRUSADE_FACTIONS_JOINED, pending_battle:attacker():faction():name()) then
				return;
			end
		end
	end
...
function CharacterTurnStart_Crusades(context)
	if context:character():has_region() then
		if table.HasValue(CURRENT_CRUSADE_TARGET_OWNED_REGIONS, context:character():region():name()) then
			if table.HasValue(CURRENT_CRUSADE_FACTIONS_JOINED, context:character():faction():name()) then
...
function CharacterEntersGarrison_Jerusalem(context)
	if MISSION_TAKE_JERUSALEM_ACTIVE and context:character():has_region() and context:character():region():name() == JERUSALEM_REGION_KEY then
		if context:character():faction():state_religion() == "att_rel_chr_catholic" then
			MISSION_TAKE_JERUSALEM_ACTIVE = false;
			Make_Peace_Crusades(cm:get_local_faction());
			cm:override_mission_succeeded_status(cm:get_local_faction(), "mk_mission_crusades_take_jerusalem_dilemma", true);
			cm:add_time_trigger("Transfer_Jerusalem_Crusades", 0.5);
			cm:remove_listener("CharacterEntersGarrison_Jerusalem");
			cm:remove_listener("MissionFailed_Jerusalem");
		end
...
function FactionReligionConverted_Crusades(context)
	local faction_name = context:faction():name();
	local state_religion = context:faction():state_religion();

	if CRUSADE_ACTIVE then
		if table.HasValue(CURRENT_CRUSADE_FACTIONS_JOINED, faction_name) and state_religion ~= "att_rel_chr_catholic" then
			Remove_Faction_From_Crusade(faction_name);
		elseif faction_name == CURRENT_CRUSADE_TARGET and state_religion == "att_rel_chr_catholic" then
			End_Crusade("aborted");
		end
	elseif MISSION_TAKE_JERUSALEM_ACTIVE then
		if context:faction():is_human() then
			MISSION_TAKE_JERUSALEM_ACTIVE = false;
			Make_Peace_Crusades(faction_name);
...
function MissionFailed_Jerusalem(context)
	local faction_name = context:faction():name();
	local mission_name = context:mission():mission_record_key();

	if mission_name == "mk_mission_crusades_take_jerusalem_dilemma" then
		MISSION_TAKE_JERUSALEM_ACTIVE = false;
		Make_Peace_Crusades(faction_name);
		cm:remove_listener("CharacterEntersGarrison_Jerusalem");
		cm:remove_listener("MissionFailed_Jerusalem");
...
function Begin_Crusade(target, owner)
	...
	local era = "early";
	local target_short = string.gsub(target, "mk_reg_", "");
...
function End_Crusade(reason)
	local target_short = string.gsub(CURRENT_CRUSADE_TARGET, "mk_reg_", "");
	...
	if reason == "won" and cm:model():world():region_manager():region_by_key(JERUSALEM_REGION_KEY):owning_faction():state_religion() == "att_rel_chr_catholic" then
		...
		elseif table.HasValue(CURRENT_CRUSADE_TARGET_OWNED_REGIONS, JERUSALEM_REGION_KEY) then
	...
	if table.HasValue(CRUSADE_REGIONS_IN_MIDDLE_EAST, CURRENT_CRUSADE_TARGET) then
		for i = 1, #CURRENT_CRUSADE_TARGET_OWNED_REGIONS do
			local region = cm:model():world():region_manager():region_by_key(CURRENT_CRUSADE_TARGET_OWNED_REGIONS[i]);
	...
	for i = 0, faction_list:num_items() - 1 do
		...
		if table.HasValue(CURRENT_CRUSADE_FACTIONS_JOINED, possible_christian_faction_name) then
			if reason == "lost" then
			...
			elseif reason == "won" then
				...
				if possible_christian_faction:is_human() == false or (possible_christian_faction:is_human() and possible_christian_faction_name == JERUSALEM_KEY or possible_christian_faction_name == PAPAL_STATES_KEY or possible_christian_faction_name == POPE_CONTROLLING_FACTION) or (possible_christian_faction:is_human() == true and possible_christian_faction ~= owner) then
					Make_Peace_Crusades(possible_christian_faction_name);
				elseif table.HasValue(CRUSADE_REGIONS_IN_MIDDLE_EAST, CURRENT_CRUSADE_TARGET) and possible_christian_faction:is_human() == true and possible_christian_faction_name == owner:name() and possible_christian_faction_name ~= JERUSALEM_KEY then
			...
			for j = 0, faction_list:num_items() - 1 do
				...
				if possible_christian_faction:allied_with(possible_ally) == true then
					if table.HasValue(CURRENT_CRUSADE_FACTIONS_JOINED, possible_ally_name) ~= true and possible_ally ~= owner and possible_ally:at_war_with(cm:model():world():faction_by_key(CURRENT_CRUSADE_TARGET_OWNER)) then
...
function Make_Peace_Crusades(faction_name)
	...
	if faction:at_war_with(target_owner) then
		...
		if not table.HasValue(CURRENT_CRUSADE_TARGET_PREEXISTING_ENEMIES, faction_name) then
			...
			for i = 0, faction_list:num_items() - 1 do
				...
				table.HasValue(FACTIONS_TO_FACTIONS_VASSALIZED[CURRENT_CRUSADE_TARGET_OWNER], possible_ally_name))) and possible_ally:at_war_with(faction) then
					Make_Peace_Crusades(possible_ally_name);
				end
			end
		end
