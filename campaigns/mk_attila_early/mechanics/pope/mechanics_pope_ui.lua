	...
	--[[if character:character_type("dignitary") and character:faction():state_religion() == "att_rel_chr_catholic" then
		...
		if CRUSADE_ACTIVE == true then
			...
			if character:faction():name() == faction_name then
				if table.HasValue(FACTION_EXCOMMUNICATED, faction_name) ~= true and character:faction():state_religion() == "att_rel_chr_catholic" then
					if character:military_force():unit_list():num_items() > 1 then
						...
						if not table.HasValue(CHARACTERS_ON_CRUSADE, LAST_CHARACTER_SELECTED:cqi()) then
...
function OnComponentLClickUp_Pope_UI(context)
	--[[if context.string == "button_college_of_cardinals" then
		...
		if not table.HasValue(CURRENT_CRUSADE_FACTIONS_JOINED, faction_name) then
...
function OnPanelOpenedCampaign_Pope_UI(context)
	if context.string == "events" then
		if CRUSADE_END_EVENT_OPEN == true then
			...
			if cm:model():world():region_manager():region_by_key(JERUSALEM_REGION_KEY):owning_faction():state_religion() == "att_rel_chr_catholic" or table.HasValue(CURRENT_CRUSADE_TARGET_OWNED_REGIONS, JERUSALEM_REGION_KEY) ~= true then
...
function SetupCardinalTemplate(character_cqi, list_box_uic)
	...
	if character then
		...
		if table.HasValue(FACTIONS_WITH_IMAGES, character_faction_name) then
