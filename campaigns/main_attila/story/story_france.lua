...
function FactionTurnStart_France(context)
	...
	if context:faction():name() == FRANCE_KEY then
		if cm:model():turn_number() == CATHAR_REVOLT_TURN then
			--cm:set_public_order_of_province_for_region("att_reg_aquitania_elusa", -200);
			--cm:force_rebellion_in_region("att_reg_aquitania_elusa", 0, cathar_force, 164, 411, true);
			--cm:force_rebellion_in_region("att_reg_aquitania_elusa", 0, cathar_force, 159, 412, true);
			CreateCivilWarArmy("att_reg_aquitania_elusa", "cathar", TOULOUSE_KEY, "fra_cathar_revolt_1", 164, 411);
			CreateCivilWarArmy("att_reg_aquitania_elusa", "cathar", TOULOUSE_KEY, "fra_cathar_revolt_2", 159, 412);
...
function CharacterEntersGarrison_Toulouse(context)
	if context:character():has_region() and context:character():region():name() == "att_reg_aquitania_elusa" and context:character():faction():name() == FRANCE_KEY and CATHAR_REVOLT_TRIGGERED == false then
...
function CharacterEntersGarrison_Normandy_France(context)	
	if context:character():has_region() and context:character():region():name() == "att_reg_lugdunensis_rotomagus" and context:character():faction():name() ~= FRANCE_KEY and FRANCE_MISSION_ACTIVE == true then
		if context:character():region():name() == "att_reg_lugdunensis_rotomagus" then
			cm:override_mission_succeeded_status(FRANCE_KEY, "mk_mission_story_france_survive_invasion", false);
